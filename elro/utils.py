...
def get_ascii(input):
    ...
    try:
        countf = 15 - len(input.encode("GBK"))
    except Exception as exception:
        logging.error(f"Unable to convert '{input}' with error: {exception}")
        return
    
    if countf < 0:
        logging.error(f"Input is to long '{input}'")
        return
